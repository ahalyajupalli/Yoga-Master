...
const ApprovedClasses = () => {
    ...
    useEffect(() => {
        if (currentUser?.email) {
            axiosSecure.get(`/classes/${currentUser.email}`)
                .then(res => {
                    console.log('Fetched classes:', res.data); // Debug: Log fetched data
                    const approvedClasses = res.data.filter(cls => cls.status === 'approved');
                    console.log('Approved classes:', approvedClasses); // Debug: Log filtered approved classes
    ...
    return (
        <div>
            <div className="my-9">
                <h1 className='text-4xl font-bold text-center '>Approved <span className='text-secondary'>Classes</span></h1>
                <div className="text-center">
                    <Fade duration={100} className='text-[12px] text-center' cascade>Here you can see all the classes that are approved.</Fade>
                </div>
                <div className="">
                    {
                        classes.length === 0 ? <div className='text-center text-2xl font-bold mt-10'>No approved classes available</div> :
                            <div className="mt-9">
                                {
                                            ...
                                            <div className="w-full">
                                                ...
                                                <div className="flex gap-5">
                                                    <div className="">
                                                        ...
                                                        <h1 className='text-secondary my-2'><span className='text-black '>Status</span> : <span className='font-bold text-green-500'>{cls.status}</span></h1>
                                                    </div>
                                                    <div className="">
